...
namespace IdentityServer.Client.Controllers
{
    ...
    {
        private const string ClientUri = @"https://localhost:44345";
        private const string CallbackEndpoint = ClientUri + @"/authorization/signInCallback";
        private const string IdServBaseUri = @"https://localhost:44330/core";
        ...
        public ActionResult SignIn()
        {
            ...
            var url = AuthorizeUri +
                ...
                "&response_type=id_token" +//в качестве ответа нам нужет id_token (это identity token), который мы потом используем для аутентификации
                "&scope=openid email profile roles" +// какой scope информации должен вернуть сервер (данный scope уточняется у пользователя при логине (данный сайт запрашивает разрешение на получение следю данных, согласны ли вы предоставить ее?))
